...
@pytest.fixture
def mock_smart_wallet_provider():
    """Create a mock Smart Wallet provider."""
    provider = Mock()
    ...
    provider.wait_for_transaction_receipt = Mock()
    provider.send_transaction = Mock()
...
def mock_smart_account():
    """Create a mock CDP smart account."""
    account = Mock()
    account.quote_swap = AsyncMock()
    account.wait_for_user_operation = AsyncMock()
    return account
...
@pytest.fixture
def action_provider():
    """Create a CDP Smart Wallet action provider instance."""
    return CdpSmartWalletActionProvider()


class TestCdpSmartWalletActionProvider:
    """Test cases for CDP Smart Wallet Action Provider."""

    def test_provider_initializes(self):
        """Test provider initializes correctly."""
        provider = cdp_smart_wallet_action_provider()
        assert isinstance(provider, CdpSmartWalletActionProvider)
        assert provider.name == "cdp_smart_wallet"

    def test_supports_network(self, action_provider):
        """Test network support returns True for all networks."""
        # Test EVM networks
    ...
    def test_get_cdp_sdk_network_mapping(self, action_provider):
    ...
    def test_get_swap_price_unsupported_network(self, action_provider, mock_smart_wallet_provider):
    ...
    def test_swap_no_owner_error(self, action_provider):
        """Test swap returns error when owner is missing."""
        # Setup mock wallet provider without _owner attribute
        mock_wallet_provider = Mock()
    ...
    def test_swap_successful_execution(
        ...
        async_result = {
            "success": True,
            "transactionHash": MOCK_SWAP_USER_OP_HASH,
            "fromAmount": "0.1",
            "fromTokenName": "ETH",
    ...
    def test_swap_unsupported_network(self, action_provider, mock_smart_wallet_provider):
...
class TestSwapSchemaForSmartWallet:
    """Test cases for SwapSchema validation with smart wallet."""
